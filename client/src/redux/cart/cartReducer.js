// Assuming this reducer is specifically for the cart,
// and you're storing the entire cart array in local storage.
  ...
  const handleCart = (state = init(), action) => {
    ...
    switch (action.type) {
      case "ADDITEM":
        ...
        const exist = state.find((item) => item.product.id === productToAdd.id);
        console.log('in addning item', action.payload, 'exist' , exist);
        if (exist) {
          // Product exists, update quantity
          newState = state.map((item) => 
            item.product.id === productToAdd.id ? { ...item, qty: item.qty + 1 } : item
          );
        } else {
      ...
      case "DELITEM":
        ...
        const existInCart = state.find((item) => item.product.id === productToRemove.id);
  
        if (existInCart.qty === 1) {
          // If only 1 qty exists, filter out the product
          newState = state.filter((item) => item.product.id !== productToRemove.id);
        } else {
          // If more than 1 qty exists, decrease the qty
          newState = state.map((item) =>
            item.product.id === productToRemove.id ? { ...item, qty: item.qty - 1 } : item
          );
        }
        break;
